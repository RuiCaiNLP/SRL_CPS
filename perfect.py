...
class SR_Model(nn.Module):
    ...
    def forward(self, batch_input, lang='En', unlabeled=False, self_constrain=False, use_bert=False, isTrain=False):
        ...
        mask_para = get_torch_variable_from_np(batch_input['mask_para'])
        mask_copy = batch_input['mask_para']
        mask_unk = get_torch_variable_from_np(batch_input['mask_unk'])
        # print(actual_lens)
        if use_bert:
        ...
        if not use_bert:
            ...
            if isTrain:
                ...
                output_word = self.SR_Matcher(pred_recur, pretrain_emb.detach(), flag_emb.detach(), word_id_emb.detach(), seq_len, copy=True,
                ...
                output_word = self.SR_Matcher(pred_recur, pretrain_emb.detach(), flag_emb.detach(), word_id_emb.detach(),
